...
PacketReceptionCallback(Ptr<const Packet> packet, uint32_t systemId)
{
    ...
    if ((*it).second.outcomes.size() > systemId - nDevices)
    {
        ...
        //  <e que essas duplicidades são removidas pela CheckReceptionByAllGWsComplete> sem certeza
        //  nisso
...
main(int argc, char* argv[])
{
    ...
    EndDevicesPlacement(cwd + "/data/placement/endDevices_LNM_Placement_" + std::to_string(seed) +
    ...
    std::string fileConfig = cwd + "/data/model/output/" + optFilePrefix +
                             "_DevicesConfigurations_" + std::to_string(seed) + "s_" +
                             std::to_string(nGateways) + "x1Gv_" + std::to_string(nDevices) +
                             "D.dat";
    ...
    if (printRates)
    {
        ...
        filePKT << "device,gateway,delay,throughput" << std::endl;
    ...
    if (printRates)
    {
        ...
        file << seed << ","
             << space2comma(tracker.CountMacPacketsGlobally(Seconds(0), appStopTime + Seconds(60)))
             << std::endl;
        file.close();
        ...
        fileG << "gwID,x,y,z,totPacketsSent,receivedPackets,interferedPackets,noMoreGwPackets,"
        ...
        for (NodeContainer::Iterator j = gateways.Begin(); j != gateways.End(); ++j)
        {
            ...
            Vector position = mobility->GetPosition();
            // gateway_id totPacketsSent receivedPackets interferedPackets noMoreGwPackets
            // underSensitivityPackets lostBecauseTxPackets
            fileG << object->GetId() << "," << position.x << "," << position.y << "," << position.z
                  << ","
                  << space2comma(tracker.PrintPhyPacketsPerGw(Seconds(0),
